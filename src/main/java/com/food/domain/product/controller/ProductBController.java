...
public class ProductBController {
	...
	public ModelAndView productDetail(@PathVariable Long id) {
		...
		System.out.println(
				"productNumber and Sales Post id : " + salesPost.getProductNumber() + " / " + salesPost.getId());
		...
		// 상품 파일 정보
		ProductFileDTO productFileInfo = productDetailMapper
				.getProductFileByProductNumber(salesPost.getProductNumber());
		...
		// 리뷰 사전 정보 가져오기
		// 리뷰 개수
		int totalReviews = (reviewMapper.countReviews(salesPost.getId()) > 0
				? reviewMapper.countReviews(salesPost.getId())
				: 0);
		...
		//문의 개수
		int inquiriesCount = inquiryMapper.countInquiries(salesPost.getId());
		mv.addObject("inquiriesCount", inquiriesCount);
		...
		List<ProductDetailCategoryInfo> categoryProducts = productDetailMapper
				.getDiscountedProducts(salesPost.getProductNumber(), categoryCode);
