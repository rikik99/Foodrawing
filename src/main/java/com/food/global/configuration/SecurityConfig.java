...
public class SecurityConfig {
	...
	public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
		http.csrf(csrf -> csrf.ignoringRequestMatchers("/sendVerificationEmail", "/verify", "/signup",
				"/invalidateSession", "/linkAccount", "/findUsername", "/verify-id-code", "/findPassword",
				"/sendPasswordResetCode", "/verify-password-code", "/passwordReset"))
				.authorizeHttpRequests(
						authorizeRequests -> authorizeRequests
								.requestMatchers("/", "/login", "/loginFail", "/signup", "/WEB-INF/views/**", "/css/**",
										"/js/**", "/images/**", "/sendVerificationEmail", "/verify", "/signupInfo",
										"/main/custompage", "/main/mainpage", "/nutrition", "/verificationSuccess",
										"/verificationFail", "/checkDuplicateUsername", "/invalidateSession",
										"/linkAccount", "/findUsername", "/verify-id-code", "/showUsername",
										"/findPassword", "/sendPasswordResetCode", "/verify-password-code",
										"/passwordReset", "/admin/loginContent")
								.permitAll().anyRequest().authenticated())
				.formLogin(formLogin -> formLogin.loginPage("/login").defaultSuccessUrl("/", true)
						.successHandler(customAuthenticationSuccessHandler).failureUrl("/loginFail").permitAll())
