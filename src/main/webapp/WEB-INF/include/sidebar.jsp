...
<div class="sidebar">
            ...
            <div class="carousel-slides">
                ...
                <c:forEach var="product" items="${recentViewedProducts}" varStatus="status">
                    ...
                    <div class="recent-product" data-salesPostId="${product.salesPostId}">
                        <a href="javascript:void(0);" onclick="submitProductForm('${product.salesPostId}')">
        ...
        document.addEventListener('DOMContentLoaded', function () {
            ...
            slidesContainer.addEventListener('click', function(event) {
                ...
                if (target.classList.contains('remove-product')) {
                    event.preventDefault();
                    const productElement = target.closest('.recent-product');
                    const salesPostId = productElement.getAttribute('data-salesPostId');
                    removeProductFromCookie(salesPostId);
                    productElement.remove();
                    updateSlider(slidesContainer, sidebarCurrentIndex); // 슬라이더 업데이트
                } else if (target.closest('.recent-product')) {
                    const productElement = target.closest('.recent-product');
                    const salesPostId = productElement.getAttribute('data-salesPostId');
                    window.location.href = '/productDetail/' + salesPostId;
                }
            });
    ...
    <script>
    let sidebarCurrentIndex = 0; // 글로벌 스코프에 변수 선언
    ...
    function submitProductForm(salesPostId) {
        const form = document.createElement('form');
        form.method = 'POST';
        form.action = '/productDetail/' + salesPostId;

        const hiddenField = document.createElement('input');
        hiddenField.type = 'hidden';
        hiddenField.name = 'salesPostId';
        hiddenField.value = salesPostId;
